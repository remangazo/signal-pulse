  ...
  async function handleDeepAnalysis() {
    ...
    try {
      ...
      const config = JSON.stringify({
        ...
        icp_ideal: icp.ideal,
        icp_not_ideal: icp.notIdeal,
        icp_signals: icp.signals,
        icp_exclude: icp.exclude,
        icp_geo: icp.geo,
        icp_budget: icp.budget,
      })
      const result = await api.saas.register({ url, name: name || undefined, config, user_id: userId || undefined })
      setAnalysis(result)
          ...
          <div className="bg-deep-card rounded-2xl border border-border p-8 min-h-[450px] transition-all duration-500">
            {step === 1 && (
              <div className="space-y-6 animate-[fade-in-up_0.4s_ease-out]">
                <div className="text-center">
                  <h2 className="text-xl font-bold text-foreground">┬┐Cu├íl es tu SaaS?</h2>
                  <p className="text-muted text-sm mt-1">Ingresa tu URL. Analizaremos tu sitio autom├íticamente.</p>
                </div>
            ...
            {step === 2 && (
              <div className="space-y-5 animate-[fade-in-up_0.4s_ease-out]">
                ...
                <div className="space-y-4">
                  ...
                  <EditableField label="┬┐Qu├⌐ hace tu producto?" value={scan.description} editing={editing.description}
                    onToggle={() => setEditing({ ...editing, description: !editing.description })}
                    onChange={(v) => setScan({ ...scan, description: v })} />
                  <EditableField label="┬┐Qu├⌐ problema resuelve?" value={scan.problem} editing={editing.problem}
                ...
                <button onClick={handleDeepAnalysis}
                  ...
                  Iniciar an├ílisis profundo ΓåÆ
                </button>
              </div>
            )}
            ...
            {step === 3 && (
              <div className="space-y-6 animate-[fade-in-up_0.4s_ease-out] text-center py-8">
                ...
                <div className="space-y-3 text-left max-w-sm mx-auto">
                  {[
                    { icon: "≡ƒÆ╛", label: "Guardando proyecto", phase: "crawling" as const },
                    { icon: "ΓÜÖ∩╕Å", label: "Optimizando informaci├│n", phase: "crawling" as const },
                    { icon: "≡ƒîÉ", label: "Buscando comunidades relevantes", phase: "analyzing" as const },
                    { icon: "≡ƒæÑ", label: "Analizando competidores", phase: "scanning" as const },
                    { icon: "≡ƒöæ", label: "Generando keywords de b├║squeda", phase: "scanning" as const },
                  ].map((item) => (
                    <div key={item.label}
                      ...
                      {(phase === item.phase || (phase === "complete" && item.icon !== "≡ƒöæ")) && (
                        <span className="ml-auto text-xs text-signal">{phase === "complete" ? "Γ£ô" : "..."}</span>
                      )}
                    </div>
                  ))}
                </div>
              ...
              <div className="space-y-5 animate-[fade-in-up_0.5s_ease-out]">
                ...
                <div className="space-y-2">
                  {discoveredCompetitors.length === 0 && !customCompetitor && (
                    <p className="text-sm text-muted text-center py-4">No se encontraron competidores autom├íticamente</p>
                  )}
                  {allCompetitors.map((c) => (
                    <div key={c}
                      ...
                      {selectedCompetitors.includes(c) && <span className="text-signal text-xs">Γ£ô</span>}
                    </div>
                  ))}
            ...
            {step === 5 && (
              <div className="space-y-5 animate-[fade-in-up_0.4s_ease-out]">
                <div className="text-center">
                  ...
                  <p className="text-muted text-sm mt-1">Esto har├í que los leads sean mucho m├ís precisos</p>
                </div>
                <div className="space-y-4">
                  <div>
                    <label className="block text-xs font-medium text-foreground mb-1">
                      Cliente Ideal <span className="text-muted font-normal">(qui├⌐n es tu mejor cliente?)</span>
                    </label>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-foreground mb-1">
                      Qu├⌐ NO es <span className="text-muted font-normal">(opcional ΓÇö ayuda a filtrar ruido)</span>
                    </label>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-foreground mb-1">
                      Se├▒ales de compra <span className="text-muted font-normal">(qu├⌐ frases indican que necesitan tu producto?)</span>
                    </label>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-foreground mb-1">
                      Palabras a ignorar <span className="text-muted font-normal">(opcional ΓÇö reduce falsos positivos)</span>
                    </label>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-xs font-medium text-foreground mb-1">Geograf├¡a</label>
                    ...
                    <div>
                      <label className="block text-xs font-medium text-foreground mb-1">Presupuesto m├¡n.</label>
            ...
            {step === 6 && (
              <div className="space-y-6 text-center animate-[fade-in-up_0.5s_ease-out]">
                <div className="text-5xl animate-bounce">≡ƒÄë</div>
                <h2 className="text-xl font-bold text-foreground">┬íSignalPulse est├í trabajando!</h2>
                ...
                <div className="bg-deep rounded-lg p-4 text-left text-xs text-muted space-y-2 border border-border">
                  ...
                  <p>≡ƒôî Producto: <span className="text-foreground">{scan.name}</span></p>
                  <p>≡ƒÄ» Descripci├│n: <span className="text-foreground">{scan.description}</span></p>
                  <p>≡ƒæÑ Competidores: <span className="text-foreground">{selectedCompetitors.join(", ") || "N/A"}</span></p>
                  <p>≡ƒÄ» ICP: <span className="text-foreground">{icp.ideal || scan.audience}</span></p>
                  {icp.notIdeal && <p>≡ƒÜ½ Excluir: <span className="text-foreground">{icp.notIdeal}</span></p>}
                  {icp.signals && <p>≡ƒôí Se├▒ales: <span className="text-foreground">{icp.signals}</span></p>}
                </div>
          ...
          <div className="hidden lg:flex items-center justify-center bg-deep-card/50 rounded-2xl border border-border p-6 min-h-[450px]">
            {step <= 2 && (
              <div className="text-center space-y-6">
                <div className="text-6xl">≡ƒôí</div>
                <h3 className="text-lg font-bold text-foreground">An├ílisis con IA</h3>
                <ul className="text-sm text-muted space-y-3 text-left">
                  <li className="flex items-start gap-2">
                    <span className="text-signal mt-0.5">Γùå</span>
                  ...
                  <li className="flex items-start gap-2">
                    <span className="text-signal mt-0.5">Γùå</span>
                    <span>Detecci├│n de competidores y keywords</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-signal mt-0.5">Γùå</span>
                    <span>B├║squeda en comunidades relevantes</span>
                  </li>
                </ul>
              </div>
            )}
  ...
  return (
    <div className="p-3 rounded-lg bg-deep border border-border">
      ...
      {editing ? (
        ...
        <p className="text-foreground text-sm">{value || "ΓÇö"}</p>
      )}
    </div>
  )
}
